...
grants_out <- grants %>%
    mutate(
        Title = paste0(
            "<a href='", AdditionalInfoURL, "' style='color:#00608a;'>", Title, "</a>"
        ),
        Deadline = as.Date(Deadline),
        Posted = as.Date(Posted),
        IsNew = Posted >= Sys.Date() - 7, # TRUE/FALSE
        New = if_else(IsNew, "<span style='color:#057872;font-size:1.2em;'>&#x25CF;</span>", ""),
...
kable(grants_out, format = tbl_format, escape = FALSE) %>%
    ...
    row_spec(break_index, background = "#ea4f12")
```